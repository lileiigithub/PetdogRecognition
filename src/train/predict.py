...
predict_probability_txt_path = "predict_probability.txt"
...
def predict_imgs(_imgs_list,_predict_batch):
...
predict_batch = 40
imgs_list_new = make_imgs_list(predict_imgs_path)
y_predict_new = predict_imgs(imgs_list_new,predict_batch)
array_max_list_new = find_array_max(y_predict_new)
#
imgs_list = np.load("imgs_list_array.npy").tolist()
y_predict = np.load("y_predict.npy")
array_max_list = find_array_max(y_predict)

for i in range(len(array_max_list_new)):
    if array_max_list_new[i][0]!=array_max_list[i][0]:
        print(array_max_list_new[i],array_max_list[i])
